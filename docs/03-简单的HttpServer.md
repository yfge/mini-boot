...
即引入一个`ThreadPool`来进行执行请求,同时将原有的处理逻辑抽象成`ServletRunnable`的类。
`ServletRunnable`这个类名字听起来或许**有些不那么靠谱**,但是我们目前为止似乎只看到了这些,所以,更改后的`ServletHandler`变成了下面的样子:
...
1. 端口监听和服务启动
2. 我们将处理请求的逻辑移到了`ServerRunnable`这个类中,意味着对Http的一些扩展性操作我们只要和`ServerRunnable`打交道就可以了。
## 定义注解
